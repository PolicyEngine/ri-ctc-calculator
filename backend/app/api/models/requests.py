...
def empty_filing_status_values() -> Dict[str, float]:
    return {
        "SINGLE": 0,
        "JOINT": 0,
        "HEAD_OF_HOUSEHOLD": 0,
...
class ReformParams(BaseModel):
    ...
    ctc_stepped_phaseout_thresholds: Dict[str, float] = Field(
        default_factory=empty_filing_status_values,
        description="Stepped phase-out thresholds by filing status"
    )
    ctc_stepped_phaseout_increments: Dict[str, float] = Field(
        default_factory=empty_filing_status_values,
        description="Stepped phase-out increments by filing status"
    )
